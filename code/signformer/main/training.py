...
class TrainManager:
    ...
    def __init__(self, model: SignModel, config: dict) -> None:
        ...
        # early stopping: stop if best does not improve for N epochs
        self.early_stopping_patience = train_config.get(
            "early_stopping_patience", 10
        )
        self.best_ckpt_epoch = 0  # epoch where the last best was found

        # periodic checkpoint saving (every N epochs)
        self.save_every_n_epochs = train_config.get("save_every_n_epochs", 5)
    ...
    def _save_periodic_checkpoint(self, epoch_no: int) -> None:
        """Save a full-state periodic checkpoint for training resume.

        Contains model, optimizer, scheduler, and all training counters
        so training can resume from exactly this point.
        """
        resume_path = os.path.join(self.model_dir, "resume.ckpt")
        state = {
            "steps": self.steps,
            "epoch": epoch_no,
            "total_txt_tokens": self.total_txt_tokens if self.do_translation else 0,
        ...
        self.logger.info(
            "Periodic checkpoint saved at epoch %d, step %d -> %s",
            epoch_no + 1, self.steps, resume_path,
        )

    def init_from_resume_checkpoint(self) -> bool:
        """Try to resume training from a periodic checkpoint.

        Returns True if a resume checkpoint was loaded, False otherwise.
        """
        resume_path = os.path.join(self.model_dir, "resume.ckpt")
        if not os.path.exists(resume_path):
            return False

        self.logger.info("Found resume checkpoint: %s", resume_path)
        ckpt = load_checkpoint(path=resume_path, use_cuda=self.use_cuda)
        ...
        self.total_txt_tokens = ckpt["total_txt_tokens"]
        self.total_gls_tokens = ckpt["total_gls_tokens"]
        self.best_ckpt_score = ckpt["best_ckpt_score"]
        self.best_all_ckpt_scores = ckpt["best_all_ckpt_scores"]
        self.best_ckpt_iteration = ckpt["best_ckpt_iteration"]
        self.best_ckpt_epoch = ckpt.get("best_ckpt_epoch", 0)
        ...
        self.logger.info(
            "Resumed from epoch %d, step %d, best score %.4f",
            self.resume_epoch + 1, self.steps, self.best_ckpt_score,
        )
        return True
    ...
    def train_and_validate(self,  train_data: Dataset, valid_data: Dataset) -> None:
        ...
        start_epoch = getattr(self, "resume_epoch", -1) + 1
        epoch_no = None
        for epoch_no in range(start_epoch, self.epochs):
            ...
            for batch in iter(train_iter):
                ...
                if self.steps % self.validation_freq == 0 and update:
                    ...
                    if self.is_best(ckpt_score):
                        ...
                        self.best_ckpt_epoch = epoch_no  # for early stopping
                        self.logger.info(
            ...
            if (epoch_no + 1) % self.save_every_n_epochs == 0:
                self._save_periodic_checkpoint(epoch_no)

            # early stopping: no improvement for N epochs
            epochs_no_improve = epoch_no - self.best_ckpt_epoch
...
def train(cfg_file: str) -> None:
    ...
    # try to resume from periodic checkpoint (crash recovery)
    if not trainer.init_from_resume_checkpoint():
        trainer.logger.info("No resume checkpoint found, starting from scratch.")
    ...
    # plot training loss + WER curves (from validations.txt)
    try:
        png = plot_training_curves(trainer.model_dir)
        if png:
            trainer.logger.info("Training plots saved to: %s", png)
    except Exception as e:  # never block training for a plot
        trainer.logger.warning("Could not generate training plots: %s", e)
